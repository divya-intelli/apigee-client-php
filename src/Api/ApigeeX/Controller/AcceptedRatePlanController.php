...
abstract class AcceptedRatePlanController extends OrganizationAwareEntityController implements AcceptedRatePlanControllerInterface
{
    ...
    private function getAcceptedRatePlans(array $query_params = []): array
    {
        ...
        foreach ($this->getRawList($this->getAcceptedRatePlansEndpoint()->withQuery(http_build_query($query_params))) as $item) {
            // Added id as name is considered as id
            $item->id = (!isset($item->id)) ? $item->name : '';
            // Need to convert time to seconds as the timestamp in response is in milliseconds
            $item->createdAt = (isset($item->createdAt)) ? (date('m/d/Y H:i:s', $item->createdAt / 1000)) : '';
            $item->lastModifiedAt = (isset($item->lastModifiedAt)) ? (date('m/d/Y H:i:s', $item->lastModifiedAt / 1000)) : '';
            $item->startTime = (isset($item->startTime)) ? (date('m/d/Y H:i:s', $item->startTime / 1000)) : '';
            if(isset($item->endTime)){
                $item->endTime = date('m/d/Y H:i:s', $item->endTime / 1000);
            }
